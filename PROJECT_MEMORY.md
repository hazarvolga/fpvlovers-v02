...
- Automatic crawl failover repair started on 2026-07-14: production Coolify was missing both `CRAWL4AI_*_HEALTH_URL` entries even though crawl URLs were present, so backup health checks fell back to the unreachable `crawler-backup:3002` alias. Production now has primary `http://crawler-proxy:3002/health` and backup `http://141.148.206.187/c4ai/health` configured through Coolify; source code also derives `/health` from a configured crawl URL to prevent future config drift. Controlled provider-only POSTs from the live app container returned `200` for both providers without queue or Dify writes. Crawl worker errors now preserve bounded provider details and permanently fail anti-bot/Cloudflare/HTTP-403 targets instead of retrying them as throttled jobs; `npm run crawl:health-test` and the extended `npm run crawl:worker-test` pass. Two historical 500s were confirmed as target anti-bot blocks, not crawler service outages.
...
- Final GAP closure snapshot on 2026-07-14: local `main` is clean and synchronized with `origin/main` at `b6f0b89`; release gates, typecheck, lint, content/metadata/media/route/language/spec-trust/editorial/social/crawl tests pass. Production is healthy on image `2709840...`; `/api/health`, homepage, hubs, and selected article routes return `200`. Shadow publishing reports `122` rows and `4` publishes in the last 24 hours; content stale jobs are `0`. Remaining operational debt is explicit: four pending crawl sources, two provider-failure throttled rows awaiting a controlled real worker window, and five runtime-generated shadow rows not present in Git. These are monitored states, not silent failures.
