...
- **Production Automation Snapshot:** Production DB check after deploy shows `crawl_jobs`: `completed=40`, `retired=9`, `pending=0`, `failed=0`, `throttled=0`. `content_jobs`: `published=66`, `queued=1`, `failed=46`; 39 failed rows are historical stale-job recovery records and 7 are old/no-error terminal rows. They are not currently blocking generation.
- **Content Job Retirement Model:** `retired` is now a first-class `ContentJobStatus`; retired jobs are terminal, do not block new brief selection, and render separately from active failed jobs in the admin table. `scripts/retire-resolved-content-jobs.ts` provides a dry-run/apply utility for preserving old failed job data while moving resolved historical rows to `retired`.
