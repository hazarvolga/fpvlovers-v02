...
- **Production Automation Snapshot:** Production DB check after deploy shows `crawl_jobs`: `completed=40`, `retired=9`, `pending=0`, `failed=0`, `throttled=0`. `content_jobs`: `published=66`, `queued=1`, `failed=46`; 39 failed rows are historical stale-job recovery records and 7 are old/no-error terminal rows. They are not currently blocking generation, but content-job retirement semantics should be formalized before mutating historical status values.
