...
- Crawl retry exhaustion hardening completed on 2026-07-14: `resolveCrawlRetryStatus()` now moves a job to terminal `failed` after the configured retry budget instead of leaving it permanently `throttled` with no next attempt. Added `npm run crawl:retry-test`; retry, worker, typecheck, lint, quality, content audit, and diff checks pass. Production read-only audit found four old pending Oscar Liang sources and two provider-failure throttled jobs (`foxeer.com`, `forum.donanimarsivi.com`); no live crawl was started because embedding budget protection remains active. The code requires deployment before those two terminal transitions can be observed.
