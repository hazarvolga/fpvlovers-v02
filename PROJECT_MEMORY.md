...
- **2026-08-02 image-cache persistence — RESOLVED, two-layer root cause:** `backfill-images/route.ts` and `publish-artifact.ts` write harvested images to `public/images/source-cache/` on the container's own disk; `docker inspect` originally showed `Mounts: []`. Cross-referencing the DB against actually-existing files proved all 87 `published_articles_shadow` rows pointing at `/api/images/source-cache/...` were phantom (file missing both on disk and in git) — every redeploy silently wiped newly-harvested real photos, from both manual backfill and the normal autonomous publish pipeline. **Layer 1:** user added a Coolify persistent volume (Source `/data/coolify/applications/r0c44ok0cskc800gs0c8o8wk/source-cache`, Destination `/app/public/images/source-cache`) and redeployed; confirmed live via `docker inspect`. **Layer 2 (found right after):** the host directory Coolify created was `root:root` 0755; the app runs as `nextjs` uid 1001/gid 65533, so every write threw `EACCES`, and the route silently fell back to hotlinking while still reporting `status: 'updated'` — first post-volume backfill claimed "87 updated" with zero files actually written. Fixed with `chown -R 1001:65533` + `chmod -R 755` on the host directory, verified with a real `touch` as uid 1001 before trusting it. Also fixed in code (`85e797e`): the route now reports a distinct `updated_hotlink_fallback` status instead of silently degrading to `'updated'`, and logs the underlying fs error. **Final verification after re-running Backfill Images:** container-side and host-side file counts both = 87 (proves the persistent mount is doing its job), and a live `curl https://fpvlovers.com.tr/api/images/source-cache/acro-stick-control-drills-for-fpv-beginners-cover-1-5fad8bf2.png` returned `HTTP 200, 32943 bytes, image/png`. Lesson for future agents: this DB/UI "success" was wrong twice in one day for two different reasons — never trust an "N updated" backfill summary without independently checking file existence on disk.
...
- **Brutal GAP Raporları:** `docs/gap-reports/archive/2026/2026-06-25-project-operations-gap-report.md` ve `docs/gap-reports/archive/2026/2026-06-25-design-ux-gap-report.md` oluşturuldu; son 15 gün commitleri, audit çıktıları, commercial içerik envanteri, route/metadata/media durumu ve UI/trust riskleri kanıtlı şekilde raporlandı.
...
### 2026-07-23 Homepage Media, Cron Generation, and Affiliate Readiness Closure

- **Homepage Görsel Kök Nedeni:** Canlı homepage son içeriklerinde source-backed media bulunmayan Dify üretimleri `/api/content/media/cover/[slug]` kapaklarına düşüyordu. Bu dürüst bir fallback'ti, fakat homepage gridinde tekrar eden/placeholder hissi yaratıyordu. Commercial içerikler için source-backed media kuralı korunurken, recent homepage kartları için generated API cover yerine kategori/topic bazlı yerel FPVLovers fallback seçimi eklendi.
- **Media Patch:** `src/lib/homepage/homepage-media.ts` oluşturuldu; `src/lib/homepage/homepage-content.ts` homepage kartlarında generated cover'ları display fallback'e yönlendiriyor. `scripts/media-policy-audit.ts` recent homepage adaylarında artifact cover yerine gerçek homepage display cover'ını denetliyor. Commercial/buyer-intent artifactlerde kaynak görsel zorunluluğu gevşetilmedi.
- **Cron/Auth ve Backlog Düzeltmeleri:** `fix(automation): unblock content backlog retries` ve `fix(cron): align middleware secret handling` commitleri ile failed content job kayıtları artık first-wave autonomous backlog seçimini bloke etmiyor. Middleware `Authorization: Bearer`, `x-cron-secret` ve `cron_secret` query biçimlerini aynı `CRON_SECRET` doğrulamasına bağlıyor.
- **Gerçek Üretim Kanıtı:** Production `/api/admin/cron/generate?count=1` gerçek Dify workflow'u ile 4 içerik üretti ve yayınladı: `fpv-drone-won-t-arm-betaflight-troubleshooting-checklist`, `receiver-tab-no-movement-in-betaflight-elrs-and-radio-checks`, `fpv-motor-spins-the-wrong-direction-safe-fix-checklist`, `fpv-vtx-overheats-on-the-bench-cooling-and-setup-checklist`. Production automation status `overall=ok`, `publishedLast24h=4`, `publishedLast14d=19`, `dailyAverage14d=1.36`, `targetMet14d=false` döndü.
- **Shadow Parity:** Production container'da üretilen JSON/MD artifactler lokale senkronize edildi. `npm run production:shadow-parity` sonucu: local 137, live 137, `liveOnly=0`, `localOnly=0`.
- **Metadata ve Internal Linking Kapatması:** Daha önce metadata bloğu olmayan 20 artifact deterministik, claim üretmeyen sınıflandırma ile zenginleştirildi. Commercial olarak sınıflandırılan buyer/comparison içeriklere en az iki valid `/article/...` internal link eklendi; troubleshooting içerikler commercial sınıftan çıkarıldı. `metadata:audit` artık 137/137 valid, 0 missing.
- **Affiliate Application Readiness:** `npm run affiliate:application-readiness` sonucu application readiness score 100. Aktif affiliate URL doğrulaması hâlâ kapalı (`ctaActivationReady=false`) çünkü gerçek affiliate program kabulü/verified URL yok; bu beklenen ve dürüst politika sınırıdır. Bu aşama monetizasyon fazında gerçek program başvuruları ve verified URLs ile devam edecek.
- **Kalan Uzun Ölçüm Blocker'ı:** 4 içerik/gün hedefi bugün production'da kanıtlandı, fakat 14 günlük güvenilirlik penceresi tek oturumda kanıtlanamaz. En az 14 gün boyunca automation status'taki `targetMet14d` izlenmeli.
- **Source-Backed Media Cache Closure:** Repeating homepage/commercial image fallback root cause was stale/dead third-party product image URLs. `scripts/source-cache-commercial-media.ts` now downloads source-backed commercial/recent product visuals into `public/images/source-cache/`, stores provenance (`originalSrc`, `sourceImageUrl`, `sourceUrl`, `credit`, `license`, `kind: source-backed-cache`), rewrites JSON/MD artifacts, and `npm run media:audit` fails commercial/buyer-intent artifacts that still hotlink external product media or lack local source-cache covers/galleries.
- **Production Deploy & Browser Verification:** Commit `8c45dd0` was pushed and deployed through Coolify deployment `m8csk44cg4w0040kk4gckwo4` on 2026-07-23. New production image `r0c44ok0cskc800gs0c8o8wk:8c45dd0b4812389db45f5c3b03973a3399f78e2c` is healthy. Production smoke passed 7/7, shadow parity is `localArticles=137`, `liveArticles=137`, `liveOnly=0`, `localOnly=0`, and a live source-cache asset returned HTTP 200.
- **Live Homepage Image Evidence:** Browser audit after full lazy-load scroll on `https://fpvlovers.com.tr/` found `totalImages=22`, `loadedImages=22`, `realBrokenImages=0`; Latest Content has `latestImages=12`, `latestLoaded=12`, `source-cache=8`, `local-fallback=4`, `generated-api=0`, `external=0`. The remaining four local fallbacks are the newest autonomous troubleshooting articles without source-backed product media, not broken placeholders.
- **Production Automation Snapshot:** Production DB check after final deploy shows `crawl_jobs`: `completed=40`, `retired=9`, `pending=0`, `failed=0`, `throttled=0`. `content_jobs`: `published=66`, `queued=1`, `retired=46`, `failed=0`. The 46 historical failed rows were moved to `retired` without deleting data.
- **Content Job Retirement Model:** `retired` is now a first-class `ContentJobStatus`; retired jobs are terminal, do not block new brief selection, and render separately from active failed jobs in the admin table. `scripts/retire-resolved-content-jobs.ts` provides a dry-run/apply utility for preserving old failed job data while moving resolved historical rows to `retired`. Production retirement was applied on 2026-07-23 and logged as automation run `retired_resolved_content_jobs` with `retired=46`.
