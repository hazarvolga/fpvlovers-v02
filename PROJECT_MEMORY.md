...
- Automation monitor phase completed locally on 2026-07-06. Added `src/lib/automation/automation-status.ts`, protected `GET /api/admin/automation/status`, and `npm run automation:status` with a default daily target of 4 publishable articles. The monitor reports content/crawl status buckets, last automation runs, latest published shadow article, 24-hour publish count, stale queued/generating jobs, crawl throttling, and actionable findings. Local `npx tsc --noEmit` and targeted ESLint passed; local CLI execution cannot reach Coolify-only PostgreSQL hostname `yo4kkoc08kgw080k404w440w`, so production API/script verification must happen after deploy or from the server network.
