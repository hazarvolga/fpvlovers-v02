...
export function SubpageShell({ children, className = '' }: { children: ReactNode; className?: string }) {
  return (
    <div className={`mx-auto max-w-[112rem] px-5 pb-24 pt-28 sm:px-8 lg:px-16 ${className}`}>
      {children}
    </div>
  );
}
...
export function SubpageHero({
  ...
  return (
    <section className="relative overflow-hidden rounded-[0.9rem] border border-white/10 bg-[#050607] shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]">
      <Image
        src={image}
        alt={imageAlt}
        fill
      ...
      />
      <div className="absolute inset-0 bg-[linear-gradient(90deg,#050607_0%,rgba(5,6,7,0.92)_34%,rgba(5,6,7,0.42)_68%,rgba(5,6,7,0.74)_100%)]" />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_16%_16%,rgba(225,34,39,0.18),transparent_24rem),radial-gradient(circle_at_80%_0%,rgba(0,242,255,0.10),transparent_28rem)]" />
      <div className="relative z-10 min-h-[24rem] p-7 sm:p-10 lg:p-14">
        <div className="max-w-3xl">
          <p className="font-mono text-[11px] font-black uppercase tracking-[0.24em] text-[#ff3131]">{label}</p>
              ...
              {actions.map((action, index) => (
                <Link
                  ...
                  className={`group inline-flex items-center justify-between gap-4 rounded-[0.45rem] border px-5 py-3 text-xs font-black uppercase tracking-[0.12em] transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] ${
                    index === 0
                      ? 'border-[#ff3131]/60 bg-[#e12227] text-white hover:bg-[#ff3131]'
        ...
        {stats.length > 0 ? (
          <div className="mt-10 grid max-w-4xl gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {stats.map((stat) => (
              <div key={stat.label} className="rounded-[0.55rem] border border-white/10 bg-black/[0.28] p-4">
...
export function SubpageSectionHeader({
  ...
  return (
    <div className="mb-6 flex items-end justify-between gap-4">
      <div>
        {label ? <p className="font-mono text-[10px] uppercase tracking-[0.24em] text-zinc-500">{label}</p> : null}
...
export function SubpageFeatureCard({
  ...
  const accentClass = {
    ...
    cyan: 'text-[#00f2ff] bg-[#00f2ff]/[0.08] border-[#00f2ff]/25 group-hover:border-[#00f2ff]/45',
  ...
  return (
    <Link href={href} className="group block h-full rounded-[0.7rem] border border-white/10 bg-[#0b0d10]/[0.92] p-6 transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] hover:-translate-y-1 hover:bg-white/[0.045]">
