...
import { analyzeIntent, decideInjections } from '@/lib/monetizationOrchestrator';
...
} from '@/lib/master-routing-tables';
...
export function orchestrate(req: MasterRequest): MasterResponse {
  ...
  if (fileRoute) {
    const app = findApp(fileRoute.appToken) || null;
  ...
  if (forceIntent) {
    ...
    const app = (forceApp ? findApp(forceApp) : (route ? findApp(route.appToken) : null)) || null;
  ...
  const app = route ? findApp(route.appToken) || null : null;
  ...
  if (route?.intent === 'regulations' && route.fallbackDatasetId === null) {
    if (intentResult.confidence < 0.3) {
      return buildResponse(query, 'regulations', app?.name || 'FPV Expert', app, route ? [route] : [], 'low_confidence_regulation', intentResult.confidence, true);
    }
  }
