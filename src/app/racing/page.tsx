...
const futureSystems = ['Live Timing', 'Leaderboards', 'Event Registration', 'Fantasy Racing', 'Race Predictor'];
...
function StatusDot({ tone = 'cyan' }: { tone?: 'cyan' | 'orange' | 'green' }) {
          ...
          <aside className="hidden xl:block">
            <div className="sticky top-28 rounded-md border border-white/10 bg-[#08090d]/88 p-3">
              <div className="mb-4 border-b border-white/10 pb-3">
                ...
                <div className="mt-2 flex items-center gap-2 text-sm font-black uppercase text-white">
                ...
                {railSections.map((section) => {
                  ...
                  return (
                    <Link
                      key={section.slug}
                      href={section.href}
                      className="group flex items-center gap-2 rounded-sm px-2.5 py-2 text-xs font-semibold text-[#aaa49c] transition-colors hover:bg-[#ff5a1f]/12 hover:text-white"
                    >
                      <Icon className="h-3.5 w-3.5 text-[#77736d] transition-colors group-hover:text-[#ff9b71]" />
                      {section.title}
                    </Link>
                  );
                })}
