...
function buildDifyPrompt(input: HardwarePayload, guardrail: string): string {
  return [
    ...
    '',
    'CRITICAL RULES:',
    '- Only make claims that are directly supported by the local guardrail data provided below.',
    '- When a guardrail check shows "warn" or missing evidence, write "unverified — confirm manually" instead of inventing a specific rating.',
    '- Do NOT invent specific numbers, product specs, or compatibility ratings that are not present in the guardrail.',
    '- If you cannot find evidence for a claim in the guardrail context, explicitly say "insufficient evidence".',
    '',
    `Frame: ${input.frame}`,
    `Motor: ${input.motor}`,
    `ESC: ${input.esc}`,
