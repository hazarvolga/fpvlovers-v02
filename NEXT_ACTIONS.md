...
0.2. **DONE - Crawl retry exhaustion fix:** crawl stores now mark exhausted retries as terminal `failed` instead of immortal `throttled`; `npm run crawl:retry-test` and `npm run crawl:worker-test` pass. Deploy this commit, then recheck the two provider-failure rows without starting a real crawl.
