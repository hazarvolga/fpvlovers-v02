...
0.4. **DONE - Homepage content gateway repair:** commit `72d08fe` implements the five-minute ISR/revalidation path, six-card featured-plus-supporting editorial layout, visible archive CTA, data-backed archive count, compact race status, zero-view suppression, and mobile-safe card geometry. Local desktop/mobile browser QA and release gates passed. Coolify deploy is intentionally still separate and must be explicitly requested.
