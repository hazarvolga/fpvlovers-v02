...
5. **NEXT - Persist `public/images/source-cache/` across deploys (root cause found, fix pending user action):** both `backfill-images/route.ts` and `publish-artifact.ts` write harvested cover/gallery images to `path.join(process.cwd(), 'public', 'images', 'source-cache')` on the **container's own ephemeral disk** — `docker inspect` on the running app container shows `Mounts: []`, no persistent volume. Cross-referencing the DB (`fpvlovers_app.published_articles_shadow`) against actually-existing files proved **all 87** articles whose `coverImage.src` pointed at `/api/images/source-cache/...` were phantom references — the real, correctly-cached photos are the 220 files from an earlier batch that happen to be git-tracked; the 87 from the most recent "real" Backfill Images run were never committed and were wiped by the next redeploy. This also silently affects every new article the normal autonomous publish pipeline harvests a real photo for, not just backfill. **Fix in progress:** user is adding a Coolify persistent volume — Name `source-cache`, Source `/data/coolify/applications/r0c44ok0cskc800gs0c8o8wk/source-cache`, Destination `/app/public/images/source-cache`. After it's added: redeploy once, then re-run **Backfill Images** (real run, not dry-run) from the admin panel — this time it should stick. Do not report the image pipeline fixed until file-existence is re-verified on disk after that redeploy, not just the DB `src` field pattern.
...
0. Add a Coolify persistent volume for the app (`r0c44ok0cskc800gs0c8o8wk`): Source `/data/coolify/applications/r0c44ok0cskc800gs0c8o8wk/source-cache`, Destination `/app/public/images/source-cache`. Redeploy, then re-run **Backfill Images** (real run) from the admin panel and re-verify file existence on disk, not just the DB `src` field — see 2026-08-02 priority section above for the full root-cause evidence.
0. Wire `src/app/admin/page.tsx`'s "Pilot Registry" table and usage chart to real `newsletter_subscribers`/`published_articles_shadow` data instead of the current hardcoded demo arrays (`subscribersData`, `usageVsGrowthData`).
