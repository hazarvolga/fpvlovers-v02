...
0.4. **NEW - Homepage content gateway repair:** read `docs/gap-reports/2026-07-14-homepage-content-presentation-ux-report.md`. Implement freshness first (`force-dynamic` or short ISR plus publish `revalidatePath('/')`), then replace the three-card bottleneck with a six-card editorial layout, visible archive CTA, dynamic content count, compact race status, and mobile-safe card geometry. Preserve admin, crawler, editorial governance, and review evidence boundaries.
