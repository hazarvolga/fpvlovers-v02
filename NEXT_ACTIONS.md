...
5. **IN PROGRESS - Product catalog evidence:** current catalog URLs are source links, not verified affiliate relationships; seed rows remain quarantined/source-pending until manufacturer/retailer evidence and network tracking are verified. Add only real affiliate URLs after account approval; do not enrich with guessed tags.
...
7. **DONE - Provenance and CTA/mobile guardrails:** `b896114` adds source references to new artifacts/trust panels, fail-closed raw storage, safe external URL checks, no placeholder affiliate blocks, mobile Search routing, and visible subpage CTAs. Re-run live 360x800/390x844 browser QA after the next UI release.
8. **Next - Restore rehearsal and source replacements:** test one backup restore into an isolated database, then replace retired anti-bot/500 URLs with accessible official/manual sources before any controlled crawl window.
