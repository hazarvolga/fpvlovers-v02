...
8. **IN PROGRESS - Attach real source hints to autonomous briefs:** the live retry was correctly held by the quality gate because the enqueued brief had no URL source evidence. Code now maps content-plan categories to trusted source URLs in `src/lib/content-automation/brief-from-source.ts`; commit, push, deploy, refresh the held job or enqueue a fresh one, then retry generation.
9. **NEXT - Validate four-per-day generation target:** after the source-hint deploy, run one real generate and inspect monitor output. Monitor should move from `generate_stale` to active runs and eventually from `publishedLast24h=0` toward `4`.
10. **Embedding budget guard:** recent crawl throttling shows `Daily embedding budget exceeded (500+500/500)`. The crawl cadence/content volume plan must not spend embedding budget blindly; prioritize source quality and daily quota visibility.
